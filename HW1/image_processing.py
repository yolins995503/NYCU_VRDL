...
def cv_show_image(title, image):
    ...
    if channels==3:
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR) # 將BGR轉為RGB
...
def fast_read_image_roi(filename, orig_rect, ImreadModes=cv2.IMREAD_COLOR, normalization=False):
    ...
    快速讀取圖片的方法
    :param filename: 圖片路徑
    :param orig_rect:原始圖片的感興趣區域rect
    ...
    :return: 返回感興趣區域ROI
    '''
    # 當採用IMREAD_REDUCED模式時，對應rect也需要縮放
    scale=1
...
def resize_image(image,resize_height, resize_width):
    ...
    width=image_shape[1]
    if (resize_height is None) and (resize_width is None):#錯誤寫法：resize_height and resize_width is None
        return image
...
def scale_rect(orig_rect,orig_shape,dest_shape):
    '''
    對影象進行縮放時，對應的rectangle也要進行縮放
    :param orig_rect: 原始影象的rect=[x,y,w,h]
    :param orig_shape: 原始影象的維度shape=[h,w]
    :param dest_shape: 縮放後影象的維度shape=[h,w]
    :return: 經過縮放後的rectangle
    '''
...
def save_image(image_path, rgb_image,toUINT8=True):
    ...
    if len(rgb_image.shape) == 2: # 若是灰度圖則轉為三通道
